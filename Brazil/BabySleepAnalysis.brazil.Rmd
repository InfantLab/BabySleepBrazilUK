...
To install the right R packages, first run the [`BabySleepAnalysis.Rmd`](".\..\BabySleepAnalysis.Rmd").


## Loading & recoding the data

All the raw data from the 10 day sleep diary is in the file `10DAY BABY SLEEP SURVEY.sav`. 
Steps:

1. We load this data
2. Classify babies who were in absorbent and ordinary diapers
3. Calculate the scores for the temprament questionnaire (IBQ-R - Rothbart et al, 2003).

```{r load-data}
#load data
dataset= read_spss("10DAY BABY SLEEP SURVEY.sav")
```

Next we calculate the babies ages on the first day of the survey (2015-06-20).
```{r ages}
#baby ages
surveyStartDate <-as.Date('20/06/2015',format='%d/%m/%Y')
baseDateTime <-as.POSIXct('2015-06-20 00:00:00') #used for bedtime calculations
dataset$DOB<-as.Date(paste(dataset$Q9_DIA,dataset$Q9_MES,dataset$Q9_ANO, sep='/'),format='%d/%m/%Y')
dataset$DaysOld <- as.integer(surveyStartDate-dataset$DOB)
dataset$OverOne <- dataset$DaysOld>365

ggplot(dataset, aes(DaysOld)) + geom_histogram(binwidth = 25)
```
Looking at a histogram of ages we see that there is one outlying baby who is `r max(dataset$DaysOld)`. So we remove this baby. 

```{r remove-outlier}
dataset <- dataset %>% filter(DaysOld < 800)
```

```{r diapertypes}
#sleeping in pampers?
dataset$PAMPERS<- ifelse(dataset$Q33_NOITE == "PAMPERS", "PAMPERS", "OTHER")
#sleeping in more absorbent premium brand (pamers, turm da monica or mamy poko)?
...
datahead <- dataset[,c(1:223,245:253)]
...
So we have data from `r nrow(datahead)` babies (`r count_if(2, datahead$Q8A_SEXO1)` female). The survey was filled in by `r nrow(datahead)` mothers, average age `r mean(datahead$Q5_EXATA)` +/- `r sd(datahead$Q5_EXATA)` years. The average age of the babies was `r mean(datahead$DaysOld)` +/- `r sd(datahead$DaysOld)`. 

## Sleeping times

Next we want to work out how long the babies slept each night based on a giong to sleep time and a wake time. Need to be a little careful because mostly babies go to bed one calendar day and wake the next but sometimes they go to bed after midnight so need to handle both conditions. 
```{r sleeptimes}
...
six_am<-hms(6*60*60)
dataset$bedtimehoursbefore6am <-mapply(lengthsleep,dataset$D_Q1_DORMIR,six_am)

dataset$bedtime <- baseDateTime + six_am - dataset$bedtimehoursbefore6am 
```
...
dataset$wakehour <-dataset$D_Q2_ACORDAR /3600
dataset$waketime2 <-baseDateTime + dataset$D_Q2_ACORDAR
```

```{r plot-bedtime}
ggplot(dataset, aes(bedtime)) + geom_histogram(bins = 18) + labs(title="Histogram of bed times.")
ggplot(dataset, aes(waketime2)) + geom_histogram(bins = 18) + labs(title="Histogram of wake times.")
```


Next relabel the interesting columns:
```{r relabel-diary}
...
## Averages per baby
Each family completed diary for 10 days so get their average scores.
```{r averages}
...
Do babies sleep longer in more aborbent diapers?

```{r ttest}
t.test(hourssleep ~ PREMIUM,data = datahead,var.equal = TRUE)
```

Do they wake up less often?

```{r ttest2}
t.test(numwakes ~ PREMIUM,data = datahead,var.equal = TRUE)
```
Do they wake up happier?

```{r ttest3}
t.test(happiness ~ PREMIUM,data = datahead,var.equal = TRUE)
```

Do they wake up more energetic?

```{r ttest4}
t.test(energy ~ PREMIUM,data = datahead,var.equal = TRUE)
```
